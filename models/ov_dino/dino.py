...
from models.segmentation import (DETRsegm, PostProcessPanoptic, PostProcessSegm,
                           dice_loss)
from .ov_deformable_transformer import build_ov_deformable_transformer
from .utils import sigmoid_focal_loss, MLP

from .registry import MODULE_BUILD_FUNCS
from .dn_components import prepare_for_cdn, dn_post_process, prepare_for_cdn_ov
from models.classifier import build_classifier
from .ov_criterion import OVSetCriterion
# from .matcher import build_matcher
from .ov_matcher import build_ov_matcher
from .ov_postprocess import OVPostProcess
from .base import sample_feature_rn,sample_feature_vit

class DINO(nn.Module):
    """ This is the Cross-Attention Detector module that performs object detection """
    # OVD 输出cls embedding不需要num_classes
    def __init__(self, backbone, transformer, classifier, num_queries, 
        ...
        if num_feature_levels > 1:
            num_backbone_outs = len(backbone.num_channels) - 1 # layer4 去除在外
            input_proj_list = []
    ...
    def forward(self, samples: NestedTensor, categories, gt_classes=None, targets=None, split_class=False):
        ...
        if self.args.use_text_desc:
            text_feature = self.classifier.get_descriptor(categories, self.args.descriptor_json)
            if self.training and self.args.use_wildmatch:
                wild_feature = self.classifier([self.args.pos_wildcard]) # add wildcard embed
                text_feature = torch.cat([text_feature, wild_feature])
        else:  
            if self.training and self.args.use_wildmatch:
        ...
        clip_pos_embeds = [
            pos_embeds[k] for k in pos_embeds.keys() if k != "dense" and k != "layer4" # layer4 for anchor prematching?
        ]
        ...
        if self.dn_number > 0 and self.training:
            ...
            input_query_label, input_query_bbox, attn_mask, dn_meta = prepare_for_cdn_ov(
                dn_args=(
                    targets, 
                    self.dn_number, 
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal, classes_ = \
            self.transformer(
                srcs=srcs,
                masks=masks,
        ...
        if not self.training:
            ...
            for coord in sample_box:
                if "RN" in self.args.backbone:
                    src_feature = features["layer3"] # C4 in ResNet
                    ...
                    src_feature = features["dense"] # dense feature for ViT
                    sizes = [((1 - m[0].float()).sum(), (1 - m[:, 0].float()).sum()) for m in src_feature.decompose()[1]]
                    roi_feats.append(
            ...
            final_outputs_class = (clip_outputs_class * self.args.eval_tau).softmax(dim=-1) \
...
def build_dino(args):
    ...
    if args.masks:
        raise NotImplementedError('masks?????')
